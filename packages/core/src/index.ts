...
class Node {
  constructor(public token: Token<TokenType>, public type: NodeType) {}
}
...
class BinOpNode extends Node {
  constructor(public left: Node, public op: Node, public right: Node) {
    super(op, "BinOpNode");
  }
...
class CharsNode extends Node {
  constructor(
    public token: Token<TokenType>
  ) {
    super(token, "CharsNode");
  }
}

class UnaryOpNode extends Node {
  constructor(op, expr) {
    super(op, "UnaryOpNode");
    this.token = op;
    this.op = op;
    this.expr = expr;
  }
...
export class ExpressionsInterpreter implements Interpreter {
  ...
  genericVisit(node) {
    throw new Error(
      `No ${node.constructor.name} method ${JSON.stringify(node)}`
    );
  }
